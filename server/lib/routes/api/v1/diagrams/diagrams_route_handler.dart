import 'dart:io';

import 'package:server/diagrams/implementations/diagrams_handler.dart';
import 'package:server/util/context_key.dart';
import 'package:server/util/http_method.dart';
import 'package:server/util/request_extension.dart';
import 'package:shelf/shelf.dart';

Future<Response> diagramsRouteHandler(Request request) async {
  final DiagramsHandler diagramsHandler = request.getFromContext(
    ContextKey.diagramsHandler,
  );

  final method = HttpMethod.fromString(request.method);

  return switch (method) {
    HttpMethod.get => await diagramsHandler.getDiagrams(request),
    HttpMethod.post => await diagramsHandler.saveDiagram(request),
    _ => Response(HttpStatus.methodNotAllowed),
  };
}
